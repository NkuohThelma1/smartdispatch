"""user-service: identity, JWT auth, credibility, contacts.
...
app = FastAPI(title="smartdispatch-user-service")
app.mount("/metrics", make_asgi_app())
SIGNUPS = Counter("smartdispatch_user_signups_total", "Signups")
LOGINS = Counter("smartdispatch_user_logins_total", "Logins")


class SignupIn(BaseModel):
    phone: str = Field(min_length=6)
    password: str = Field(min_length=6)
    role: str = Field(pattern="^(customer|agent|business)$")
...
async def signup(body: SignupIn):
    ...
    await publish(
        "user.registered",
        {"id": uid, "phone": body.phone, "role": body.role},
        key=uid
    )
    SIGNUPS.inc()
