...
class TestAnalyticsServiceConsume(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.events = _load_events_module("analytics-service")
    ...
    async def test_consume_handler_success_commits_and_records_success(self):
        msgs = [FakeMsg("order.placed", 0, 1, "k1", {"_stream": "order.placed", "order_id": "o1"})]
        ...
        await self.events.consume(["order.placed"], "ag", handler)
