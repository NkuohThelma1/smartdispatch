...
class TestUserServiceConsume(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.events = _load_events_module("user-service")
    ...
    async def test_consume_handler_success_commits_and_records_success(self):
        msgs = [FakeMsg("user.registered", 0, 1, "k1", {"_stream": "user.registered", "user_id": "u1"})]
        ...
        await self.events.consume(["user.registered"], "ug", handler)
